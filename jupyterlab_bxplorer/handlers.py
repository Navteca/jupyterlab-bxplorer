...
from .custom_exceptions.CustomExceptions import (
    ODSourceNotFound,
    GitHubAPICallRateLimitExceeded,
    BucketIsNotAccessibleError,
)
...
od_sources: List[Dict[str, Any]] = [
    {
        ...
        "cache": aws_open_data_bucket_list,
        "disabled": False,
    },
    {
        "name": "Microsoft",
        "url": "",
        "cache": ms_open_data_bucket_list,
        "disabled": True,
    },
    {
        "name": "Google",
        "url": "",
        "cache": google_open_data_bucket_list,
        "disabled": True,
    },
]
...
def _get_unsigned_s3_client():
    s3 = boto3.client(
        "s3", config=Config(max_pool_connections=50, signature_version=UNSIGNED)
    )
    return s3
...
class RouteHandler(APIHandler):
    ...
    def get(self):
        self.finish(
            json.dumps({"data": "This is /jupyterlab-bxplorer/get-example endpoint!"})
        )
...
class GetChonkyFileDataHandler(APIHandler):
    ...
    def post(self):
        try:
                ...
                if od_source:
                    ...
                    if od_info:
                        ...
                        if not cache.files_list or cache.expires_on <= datetime.now():
                            ...
                            od_files_info = [
                                {
                                    "od_bucket_name": os.path.basename(
                                        e["download_url"]
                                    ).split(".")[0],
                            ...
                            with concurrent.futures.ThreadPoolExecutor(30) as executor:
                                futures = {
                                    executor.submit(
                                        self.get_od_information_from_file,
                                        od_bucket["download_url"],
                                    ): od_bucket
                                    for od_bucket in od_files_info
                                }
                                for future in concurrent.futures.as_completed(futures):
                                    data = future.result()
                                    od_sources_information.append(data)

                            od_sources_information[:] = dict(
                                (v["name"], v) for v in od_sources_information if v
                            ).values()
                            ...
                            response = [
                                ...
                                for info in od_sources_information
                                if info
                            ]
                    ...
                    if bucket == "":
                        if (
                            not buckets_list_data.files_list
                            or buckets_list_data.expires_on <= datetime.now()
                        ):
                            all_buckets = s3_client.list_buckets()

                            with concurrent.futures.ThreadPoolExecutor(
                                len(all_buckets["Buckets"])
                            ) as executor:
                                futures = {
                                    executor.submit(
                                        single_accessible_bucket_check,
                                        s3_client,
                                        bucket["Name"],
                                    ): bucket
                            ...
                            response = [
                                ChonkyFileData(
                                    ...
                                    name=bucket,
                                    isDir=True,
                                    additionalInfo=[{"type": client_type}],
                                ).as_dict()
                                for bucket in bucket_list
                            ]
                            response = sorted(response, key=lambda d: d["name"].lower())
                            buckets_list_data.files_list = response
                            buckets_list_data.expires_on = datetime.now() + timedelta(
                                days=1
                            )
                        else:
                        ...
                        chonky_file_structure = self.get_bucket_objects(
                            s3_client, bucket, new_prefix
                        )
                        response = sorted(
                            [t.as_dict() for t in chonky_file_structure],
                            key=lambda d: d["name"].lower(),
                        )

            else:
                raise BadRequest(
                    "There has been an error processing the request data. Please, check a bucket and prefix had been provided."
                )
            ...
            print(
                f"There has been an error processing the request data in {sys._getframe(  ).f_code.co_name} => {e}"
            )
        except ClientError as e:
            logger.info(f"Boto3 Client Error while listing object in bucket: {e}")
        except Exception as e:
            logger.info(
                f"There has been an unexpected error in GetChonkyFileDataHandler"
            )
        else:
    ...
    def get_bucket_objects(
        ...
        try:
            bucket_objects: ListObjectsV2OutputTypeDef = s3_client.list_objects_v2(
                Bucket=bucket, Prefix=prefix if prefix != "/" else "", Delimiter="/"
            )
            s3_objects: Union[List[ObjectTypeDef], None] = bucket_objects.get(
                "Contents", None
            )
            s3_prefixes: Union[List[CommonPrefixTypeDef], None] = bucket_objects.get(
                "CommonPrefixes", None
            )
            if s3_objects:
                chonkyFiles += [
                    ChonkyFileData(
                        ...
                        additionalInfo=[
                            {
                                "region": bucket_objects.get("ResponseMetadata", {})
                                .get("HTTPHeaders", {})
            ...
            if e.response.get("Error", {}).get("Code", None) == "NoSuchBucket":
                print(
                    f"{e.response.get('Error', {}).get('Message', 'No Message')}: {e.response.get('Error', {}).get('BucketName', 'No bucket name')}"
                )
            else:
...
class DownloadsHandler(APIHandler):
    ...
    def post(self):
        ...
        try:
            ...
            if request_data:
                ...
                if s3_uri and downloadPath:
                    with pkg_resources.path(
                        "jupyterlab_bxplorer", "cmd_launcher.py"
                    ) as p:
                        cmd_split = shlex.split(
                            f"{which('python')} {p} {s3_uri} {downloadPath} {client_type}"
                        )
                        subprocess.Popen(
                            cmd_split,
            ...
            logger.info(
                f"It was not possible to download the selected files, please contact the administrator."
            )
                ...
                if delete_all:
                    ...
                    with self.db.get_session() as session:
                        download_to_delete = (
                            ...
                            .filter(
                                Downloads.id == id_to_del, Downloads.pid == pid_to_del
                            )
                            .first()
                        )
        ...
        try:
            ...
            if request_data:
                ...
                s3_client = (
                    ...
                    if bucket_source_type.lower() in ["public", "external"]
                ...
                new_obj = Favorites(
                    path, dumps(chonky_object), bucket_source, bucket_source_type
                )
                with self.db.get_session() as session:
                    bucketFound = (
                        session.query(Favorites.path).filter_by(path=path).first()
                    )
                    if bucketFound:
                        response.status_code = 600
                        response.error.code = 600
                        response.error.message = (
                            f"The bucket {path} already exists in your favorites list."
                        )
                    else:
                        session.add(new_obj)
                        session.commit()
                        response.status_code = 200
                        response.data = (
                            f"The bucket {path} has been added to your favorites list."
                        )
            else:
            ...
            if request_data:
                with self.db.get_session() as session:
                    favorite_to_delete = (
                        ...
                        .where(Favorites.path == request_data)
                        .first()
                    )
...
class FitsHandler(APIHandler):
    ...
    def get(self):
        try:
            ...
            with fits.open(
                uri, use_fsspec=True, fsspec_kwargs=fsspec_kwargs, memmap=True
            ) as hdul:
            ...
            self.log.info(
                f"There has been an exception reading the favorites db => {e}"
            )
...
def setup_handlers(web_app):
    ...
    handlers = [
        ...
        (
            url_path_join(base_path, "/open_data/get_sources_list"),
            GetOpenDataSourcesListHandler,
        ),
